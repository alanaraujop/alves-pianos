...
export function Hero({ onOrcamento }: HeroProps) {
  return (
    <section
      id="inicio"
      className="scroll-mt-header bg-gradient-ebony px-gutter pb-section pt-8 md:pt-10"
    >
      <div className="mx-auto grid max-w-container-lg grid-cols-1 items-center gap-8 md:grid-cols-[1.05fr_.95fr] md:gap-8">
        ...
        <div className="order-2 md:order-1">
          ...
          <h1
            className={`mb-4 text-pretty ${textStyle.display} text-ivory-100`}
          >
            Seu piano merece voltar a soar como no primeiro dia.
          </h1>
          <p
            className={`mb-6 max-w-[520px] ${textStyle.lead} text-ink-onDarkMuted`}
          >
            Restauração, afinação e venda de pianos, feitas por quem escuta
            instrumento há quarenta anos. Atendemos casas, teatros e escolas de
            música.
          </p>
          ...
          <dl className="mt-7 grid grid-cols-3 gap-4 sm:gap-6">
            {STATS.map(([n, l]) => (
              <div key={l}>
                ...
                <dd className="m-0">
                  <span className="block font-display text-2xl leading-none text-gold-300">
                    {n}
                  </span>
                  <span
                    aria-hidden="true"
                    className={`mt-1 block ${textStyle.small} text-ink-onDarkMuted`}
                  >
                    {l}
                  </span>
                </dd>
